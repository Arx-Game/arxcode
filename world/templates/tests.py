...
class TemplateTests(ArxCommandTest, TemplateMixins):
    ...
    def setUp(self):
        ...
        self.c1_template = self.create_template_for(self.paccount1, title="My Test Template")
        self.c1_template.save()

        self.c2_template = self.create_template_for(self.paccount1, title="My Restricted Template", access_level='RS', apply_attribution=True)
        self.c2_template.save()

    def test_owner_can_see_their_templates(self):
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/list", self.create_table_view([self.c1_template, self.c2_template]))
    ...
    def test_anyone_can_see_open_templates(self):
        open = self.create_template_for(self.paccount1, "Other Template", access_level="OP")
        open.save()
    ...
    def test_cannot_nest_templates(self):
        self.setup_cmd(CmdTemplateForm, self.char2)
        self.call_cmd("/create", None)
        self.call_cmd("/desc This is so awesome [[TEMPLATE_", "Templates cannot be nested.")

    def test_can_get_markup(self):
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/markup {}".format(self.c1_template.id), "[[TEMPLATE_{}]]".format(self.c1_template.id))
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/markup 131".format(self.c1_template.id),
    ...
    def test_can_grant_access_to_a_template(self):
        ...
        self.call_cmd("/grant {}={}".format(self.char2.name, self.c1_template.id),
        ...
        self.call_cmd("/grant Char2=12312312312", "You do not own a template with that id.")
        self.call_cmd("/grant Char-1={}".format(self.c2_template.id), "Char-1 does not exist.")

    def test_can_revoke_access_to_a_template(self):
        TemplateGrantee(grantee=self.char2.roster, template=self.c2_template).save()

        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/revoke {}={}".format(self.char2.name, self.c2_template.id),
                      "You have revoked {}'s access to [[TEMPLATE_{}]].".format(self.char2.name, self.c2_template.id))
        ...
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/revoke {}={}".format(self.char2.name, self.c2_template.id),
                      "{} does not have access to [[TEMPLATE_{}]].".format(self.char2.name, self.c2_template.id))

        self.setup_cmd(CmdTemplateForm, self.char2)
        self.call_cmd("/list", self.create_table_view([]))

        self.call_cmd("/revoke Char2=12312312312", "You do not own a template with that id.")
        self.call_cmd("/revoke Char-1={}".format(self.c2_template.id), "Char-1 does not exist.")
    ...
    def test_changing_access_from_restricted_to_private_clears_grantees(self):
        ...
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/change_access {}={}".format(self.c2_template.id, 'PRIVATE'),
                      "You have changed the access level to PRIVATE.")

        self.assertEqual(self.c2_template.grantees.count(), 0)

        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/change_access {}={}".format(self.c2_template.id, 'RESTRICTED'),
        ...
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/change_access {}={}".format(self.c2_template.id, 'OPEN'),
    ...
    def test_can_get_list_of_grantees(self):
        self.setup_cmd(CmdTemplateForm, self.char1)
        self.call_cmd("/grantees {}".format(self.c1_template.id),
        ...
        self.call_cmd("/grantees", "Which template do you want to get the list of grantees for?")
    ...
    def test_find_template_ids(self):
        ...
        string = "This is awesome. {}. We should see more values! Like {}.".format(template_1, template_2)

        self.assertEquals(set(self.find_templates(string)), set([template_2, template_1]))
        self.assertEquals(set(self.find_template_ids(string)), set(["1", "21"]))

    def test_find_templates_from_desc(self):
        desc = "this is my awesome desc! It has {} as well as {} in it!".format(self.c1_template.markup(), self.c2_template.markup())

        self.assertEqual(set(Template.objects.in_list(self.find_template_ids(desc)).all()), set([self.c1_template, self.c2_template]))

    def test_replace_desc(self):
        desc = "this is my awesome desc! It has {} as well as {} in it!".format(self.c1_template.markup(), self.c2_template.markup())

        parsed_desc = "this is my awesome desc! It has {} as well as {} in it!".format(self.c1_template.desc, self.c2_template.desc)
