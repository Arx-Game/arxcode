...
class RPEventCreateForm(forms.ModelForm):
    ...
    player_queryset = (
        PlayerOrNpc.objects.filter(
            Q(player__roster__roster__name="Active") | Q(player__is_staff=True)
        )
        .distinct()
        .order_by("player__username")
    )
    org_queryset = (
        Organization.objects.filter(members__isnull=False).distinct().order_by("name")
    )
    ...
    location = forms.ModelChoiceField(
        queryset=ObjectDB.objects.filter(
            db_typeclass_path=settings.BASE_ROOM_TYPECLASS
        ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.owner.player.is_staff:
            ...
            self.fields["org_invites"].queryset = self.org_queryset.filter(
                Q(secret=False) | Q(id__in=current_orgs)
            )
    ...
    def check_risk(self):
        ...
        if not any(
            gm
            for gm in gms
            if gm.player.is_staff or gm.player.check_permstring("builders")
        ):
            if risk != RPEvent.NORMAL_RISK:
                self.add_error(
                    "risk",
                    "Risk cannot be altered without a staff member as GM. Set to: %r"
                    % risk,
                )
    ...
    def check_location_or_plotroom(self):
        ...
        if not (location or plotroom):
            ...
            self.add_error(
                "plotroom", "Please only specify location or plot room, not both."
            )
