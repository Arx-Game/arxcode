...
class TestCraftingCommands(ArxCommandTest):
    def setUp(self):
        ...
        self.material = CraftingMaterialType.objects.create(
            name="testonium", desc="A test material.", value=5000
        )
...
class TestCrisisCommands(ArxCommandTest):
    ...
    def test_cmd_gm_crisis(self, mock_get_week, mock_inform_staff, mock_now):
        ...
        self.call_cmd(
            "/create test crisis2/ermagerd headline=test desc",
        ...
        with patch(
            ...
            self.call_cmd(
                "/update 1/another test episode/test synopsis=test gemit 2",
                "You have updated the crisis, creating a new episode called 'another test episode'.",
            )
            mock_msg_and_post.assert_called_with(
                "test gemit 2", self.caller, episode_name="another test episode"
            )
...
class TestDomainProgression(ArxCommandTest):
    def test_hunger_and_lawlessness_weekly_adjustment(self):
        ...
        domain = Domain.objects.create(
            unassigned_serfs=expected_unassigned_serfs, stored_food=0
        )
...
class TestGeneralDominionCommands(ArxCommandTest):
    def test_admin_domain(self):
        ...
        self.call_cmd(
            "/npc_create org", "Usage: <org>=<npc group name>, <social rank #>"
        )
        self.call_cmd(
            "/npc_create org=Vixens, 7", "Social rank should be in the range of 2-6."
        )
        self.call_cmd(
            "/npc_create org=Vixens, 6", "Liege 'Orgtest' does not have a domain."
        )
    ...
    def test_cmd_work(self, mock_dice_check, mock_get_week, mock_randint):
        from world.dominion.models import Organization, AssetOwner, WorkSetting
        ...
        self.call(
            self.cmd_class(),
            args="",
            msg="Command does not exist. Please see 'help work'.",
            caller=self.caller,
            cmdstring="task",
        )
        ...
        self.call_cmd(
            "Orgtest, 5", "Type must be one of these: Economic, Military, Social."
        )
        ...
        self.call_cmd(
            "Orgtest, economic",
            "You use 15 action points and have 85 remaining this week.|"
        ...
        WorkSetting.objects.create(
            organization=org, stat="intellect", skill="war", message="Test Msg."
        )
        self.char1.traits.skills = {"economics": 1, "war": 4}
        mock_dice_check.return_value = 20
        self.call_cmd(
            "Orgtest, economic",
            "You use 15 action points and have 70 remaining this week.|"
            "Your social clout reduces difficulty by 1.\nTest Msg. "
            "Char rolling intellect and war. You have gained 6 economic resources."
            "|Orgtest has new @informs. Use @informs/org Orgtest/1 to read them.",
        )
        ...
        self.call_cmd(
            "/invest orgtest,economic=testaccount2",
            "You must specify at least 10 resources to invest.",
        )
        self.call_cmd(
            "/invest orgtest,economic,20=testaccount2",
            "You cannot afford to pay 20 resources.",
        )
        self.call_cmd(
            "/invest orgtest,economic,18=testaccount2",
        ...
        self.char2.mods.create_knack(
            "Thrifty", "intellect", "economics", desc="money smarts"
        )
        self.call_cmd(
            "Orgtest, economic=TestAccount2",
    ...
    def test_cmd_organization(self):
        ...
        old_plot = Plot.objects.create(
            name="old plot", resolved=True, usage=Plot.PLAYER_RUN_PLOT
        )
        inv = OrgPlotInvolvement.objects.create(
            plot=old_plot, org=org, rank_requirement=5
        )
        ...
        self.call_cmd(
            "/addplot test plot", "Must give name/id and rank to set the plot at."
        )
        self.call_cmd("/addplot test plot,10", "No Plot found using 'test plot'.")
        PCPlotInvolvement.objects.create(
            plot=plot, dompc=self.dompc, admin_status=PCPlotInvolvement.OWNER
        )
...
class TestPlotCommands(TestTicketMixins, ArxCommandTest):
    def setUp(self):
        ...
        self.plot2 = Plot.objects.create(
            name="testplot2", resolved=True, usage=Plot.GM_PLOT
        )
    ...
    def test_cmd_plots(self):
        ...
        self.plot1.dompc_involvement.create(
            dompc=self.dompc2,
            cast_status=PCPlotInvolvement.SUPPORTING_CAST,
        ...
        self.call_cmd(
            "/old", "Resolved Plot (ID) Involvement \n" "testplot2 (#2)     Main Cast"
        )
        ...
        self.call_cmd(
            "/createbeat 2=foo", "You lack the required permission for that plot."
        )
        self.call_cmd(
            "/createbeat 1",
        ...
        self.call_cmd(
            "/createbeat 1=asdf/", "Please have a slightly longer IC summary."
        )
        self.call_cmd(
            "/createbeat 1=Bob died it was super/...sad",
            "You have created a new beat for testplot1, ID: 2.",
        )
        prompt = (
            "[Proposed Edit to Beat #2] Bob died it was super\nOOC Char2: ...sad\nOOC Char2: but not really"
        ...
        self.call_cmd(
            "/add/rpevent 1=99", "You are not able to alter a beat of that ID."
        )
        self.call_cmd(
            "/add/rpevent 1=2", "You have added test event to beat(ID: 2) of testplot1."
        )
        beat3.delete.assert_called()
        self.assertEqual(event.beat, beat2)
        self.call_cmd(
            "/add/rpevent 1=2", "It already has been assigned to a plot beat."
        )
        flashback = Flashback.objects.create(title="test flashback")
        flashback.invite_roster(self.roster_entry2, owner=True)
        self.call_cmd(
            "/add/flashback 1=1",
            "You have added test flashback to beat(ID: 1) of testplot1.",
        )
        ...
        self.call_cmd(
            "/add/gemit 1=1", "You have added StoryEmit #1 to beat(ID: 1) of testplot1."
        )
        self.assertEqual(gemit.beat, beat1)
        self.caller = self.char2
        action = self.dompc2.actions.create(
            plot=self.plot1, status=PlotAction.PUBLISHED
        )
        self.call_cmd(
            "/add/action 1=2",
        ...
        self.call_cmd(
            "/perm 2=foo/recruiter", "You lack the required permission for that plot."
        )
        self.call_cmd("/perm 1=foo", "You must specify both a name and perm-level.")
        self.call_cmd(
            "/perm 1=foo/recruiter",
            "No one is involved in your plot by the name 'foo'.",
        )
        self.call_cmd(
            "/perm 1=testaccount2/recruiter",
            "Owners cannot have their admin permission changed.",
        )
        self.call_cmd(
            "/perm 2=foo/gm", "You lack the required permission for that plot."
        )
        self.call_cmd(
            "/perm 1=foo/gm", "No one is involved in your plot by the name 'foo'."
        )
        ...
        self.call_cmd(
            "/cast 1=testaccount/supporting",
            "You have added Testaccount to the plot's Supporting Cast members.",
        )
        self.call_cmd("/perm 1=testaccount/gm", "You have marked Testaccount as a GM.")
        self.call_cmd("/cast 1=testaccount/required", gm_err)
        self.call_cmd(
            "/perm 1=testaccount/player", "You have marked Testaccount as a Player."
        )
        self.call_cmd(
            "/perm 1=testaccount/recruiter",
            "You have marked Testaccount as a Recruiter.",
        )
        self.call_cmd(
            "/perm 1=testaccount/owner",
            "You entered 'owner'. Valid permission levels: gm, player, or recruiter. "
            "Valid cast options: required, main, supporting, or extra.",
        )
        self.call_cmd(
            "/invite 2=testaccount", "You lack the required permission for that plot."
        )
        ...
        self.call_cmd(
            "/invite 2=testaccount",
            "Must provide both a name and a status for invitation.",
        )
        self.call_cmd(
            "/invite 2=testaccount,foo",
            "Status must be one of these: required, main, supporting, " "extra",
        )
        self.call_cmd(
            "/invite 2=testaccount,extra",
            "You have invited Testaccount to join testplot2.",
        )
        self.call_cmd("/invite 2=testaccount,extra", "They are already invited.")
        self.call_cmd(
            "/rfr 2=argleblargle", "You lack the required permission for that plot."
        )
        self.call_cmd("/rfr 1", "Open tickets for testplot1:\n\nID Title")
        self.call_cmd(
            "/rfr 1=test ticket", "You have submitted a new ticket for testplot1."
        )
        self.call_cmd(
            "/rfr 1,10=test beat ticket", "You are not able to alter a beat of that ID."
        )
        self.call_cmd(
            "/rfr 1,2=test beat ticket",
            "You have submitted a new ticket for testplot1.",
        )
        self.call_cmd(
            "/storyhook 2=testaccount2",
            "You lack the required permission for that plot.",
        )
        self.call_cmd(
            "/storyhook 1=testaccount2", "You have removed Testaccount2's story hook."
        )
        self.call_cmd(
            "/storyhook 1=testaccount2/test hook",
            "You have set Testaccount2's story hook that contacts "
            "can see to: test hook",
        )
        self.caller = self.char1
        self.call_cmd("/accept", "Outstanding invitations: 2")
        self.call_cmd(
            "/accept 1", "No invitation by that ID.\nOutstanding invitations: 2"
        )
        self.call_cmd("/accept 2", "You have joined testplot2 (Plot ID: 2)")
        self.call_cmd(
            "/accept 2", "No invitation by that ID.\nOutstanding invitations:"
        )
        self.call_cmd("/leave", "No plot found by that ID.\n")
        self.call_cmd(
            "/pitch",
            "You must provide a name, a one-line summary, desc, "
            "and notes for GMs separated by '/'.",
        )
        self.call_cmd("/pitch foo/bar/desc/zeb=5", "No plot found by that ID.")
        self.call_cmd(
            "/pitch foo/bar/desc/zeb=2",
            "You made a pitch to staff for a new plot. Ticket ID: 10.",
        )
        self.call_cmd("/leave 1", "You have left testplot1.")
        self.call_cmd("/findcontact", "You do not have a secret by that number.")
        secret = Clue.objects.create(
            tangible_object=self.char1, clue_type=Clue.CHARACTER_SECRET, desc="sekrit"
        )
        secret.plot_involvement.create(
            plot=self.plot1, access=CluePlotInvolvement.HOOKED
        )
        disco = secret.discoveries.create(character=self.roster_entry)
        self.assertEqual(self.char1.messages.secrets, [disco])
        self.call_cmd(
            "/findcontact 2",
            "People you can talk to for more plot involvement with your secret:\n\n"
        ...
        self.call_cmd(
            "/addclue 2=3/foo",
            "You have associated clue 'testclue3' with plot 'testplot2'.",
        )
        self.call_cmd(
            "/add/clue 2=3/so connected", "That clue is already related to that plot."
        )
        ...
        self.call_cmd(
            "/addrevelation 2=1/foo",
            "You have associated revelation 'testrev' with plot 'testplot2'.",
        )
        self.call_cmd(
            "/add/revelation 2=1/blargh",
            "That revelation is already related to that plot.",
        )
        ...
        theory_disco = TheoryPermissions.objects.create(
            player=self.account, theory=theory
        )
        self.call_cmd(
            "/add/theory 2=1",
            "You have associated theory 'Testaccount's theory on test_theory'"
            " with plot 'testplot2'.",
        )
        self.call_cmd(
            "2",
    ...
    def test_cmd_gm_plots(self, mock_now):
        ...
        self.call_cmd(
            "/create testplot3/test/test",
            "You have created a new gm plot: testplot3 (#3).",
        )
        self.call_cmd(
            "/create testplot4/test/test=testplot3",
        ...
        self.call_cmd(
            "/addbeat/other 2=story/gm notes",
            "You have created a new beat for plot testplot2.",
        )
        update = self.plot2.updates.last()
        self.assertEqual(update.desc, "story")
        self.assertEqual(update.gm_notes, "gm notes")
        self.call_cmd(
            "/addbeat/rpevent 2=5/story 2/gm notes 2",
            "Did not find an object by that ID.",
        )
        event = RPEvent.objects.create(name="test event")
        self.call_cmd(
            "/addbeat/rpevent 2=1/story 2/gm notes 2",
        ...
        self.call_cmd(
            "/perm 4=testaccount2,gm", "You have set Testaccount2 as a GM in testplot4."
        )
        self.call_cmd(
            "/participation 4=testaccount,blargh",
            "Choice must be one of: required cast, main cast, supporting cast, extra, tangential.",
        )
        self.call_cmd(
            "/participation 4=testaccount2,required cast",
            "You have set Testaccount2 as a Required Cast in testplot4.",
        )
        pitch = create_plot_pitch(
            "desc", "notes", "testpitch", self.plot2, "headline", self.account2
        )
        self.assertEqual(pitch.plot.usage, Plot.PITCH)
        self.call_cmd(
            "/pitches",
            "ID Submitter    Name      Parent    \n"
            "8  Testaccount2 testpitch testplot2",
        )
        self.call_cmd(
            "/pitches 8",
            "[Ticket #8] Pitch: testpitch (testplot2)\nQueue: PRP Questions - Priority 3\n"
            "Player: TestAccount2\nLocation: Room (#1)\n"
        ...
        self.call_cmd(
            "/pitches/followup 8=meh", "You have added a followup to Ticket 8."
        )
        self.call_cmd(
            "/pitches/approve 8=k",
        ...
        pitch2 = create_plot_pitch(
            "desc", "notes", "testpitch2", self.plot2, "headline", self.account2
        )
        ...
        self.call_cmd(
            "/connect/asdf 5=foo/bar",
            "You must include the type of object to connect: char, clue, revelation, org.",
        )
        self.call_cmd(
            "/connect/char 5=testaccount/stuff",
            "You have connected Testaccount with testpitch.",
        )
        org = Organization.objects.create(name="testorg")
        self.call_cmd(
            "/connect/org 5=testorg/staff", "You have connected testorg with testpitch."
        )
        self.assertEqual(org.plots.first().id, 5)
        Clue.objects.create(name="testclue")
        self.call_cmd(
            "/connect/clue 5=testclue/stuff",
            "You have connected testclue with testpitch.",
        )
        Revelation.objects.create(name="testrev")
        self.call_cmd(
            "/connect/revelation 5=testrev/stuff",
            "You have connected testrev with testpitch.",
        )
        pitch3 = create_plot_pitch(
            "desc", "notes", "testrfr", self.plot2, "headline", self.account2
        )
        ...
        self.call_cmd(
            "/rfr 10",
            "[Ticket #10] Pitch: testrfr (testplot2)\nQueue: PRP Questions - Priority 3\n"
            "Player: TestAccount2\nLocation: Room (#1)\n"
            "Submitted: 08/27/78 12:08:00 - Last Update: 08/27/78 12:08:00\nRequest: notes\n"
            "Plot: testrfr (#7)\nGM Resolution: None",
        )
