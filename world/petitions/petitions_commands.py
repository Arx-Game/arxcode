...
class CmdPetition(RewardRPToolUseMixin, ArxCommand):
    ...
    def func(self):
        ...
        try:
            if self.check_switches(self.list_switches) or (
    ...
    def do_ignore_switches(self):
        ...
        if self.lhs == "all":
            ...
            self.msg(
                "You are %s informed of new petitions."
                % ("now" if settings.inform else "no longer")
            )
            settings.save()
        elif self.lhs == "general":
            settings.ignore_general ^= True
            self.msg(
                "You are %s informed of new general petitions."
                % ("no longer" if settings.ignore_general else "now")
            )
            settings.save()
        else:
            org = self.get_org_from_args(self.lhs)
...
class CmdBroker(ArxCommand):
        ...
        if "search" in self.switches and self.args:
            ...
            if sale_type in (
                ...
                if set(self.args.lower().split()) & {"materials", "mats", "crafting"}:
                    ...
                    query = Q(crafting_material_type__name__icontains=self.args) | Q(
                        owner__player__username__iexact=self.args
                    )
            qs = qs.filter(query)
    ...
    def get_amount(self, args, noun="amount"):
        ...
        try:
            ...
            raise self.BrokerError(
                "You must provide a positive number as the %s." % noun
            )
        return amount
    ...
    def make_sale_offer(self):
        ...
        if sale_type == BrokeredSale.ACTION_POINTS:
            ...
            if not self.caller.player_ob.pay_materials(material_type, amount):
                raise self.BrokerError(
                    "You don't have enough %s to put on sale." % material_type
                )
        dompc = self.caller.player_ob.Dominion

        sale, created = dompc.brokered_sales.get_or_create(
            price=price,
            sale_type=sale_type,
            crafting_material_type=material_type,
            broker_type=BrokeredSale.SALE,
        ...
        if created:
            self.msg(
                "Created a new sale of %s %s for %s silver each and %s total."
                % (amount, sale.material_name, price, sale.amount * price)
            )
        else:
            if amount > 0:
                self.msg(
                    "Added %s to the existing sale of %s for %s silver each and %s total."
    ...
    def check_for_buyers(self, sale):
        ...
        for order in buy_orders:
            if amount > 0:
                buyer = order.owner
                if (
                    order.owner.player.roster.current_account
                    != self.caller.roster.current_account
                ):
                    if amount > order.amount:
                        buyamount = order.amount
                    ...
                    amount -= buyamount
                    self.msg(
                        "You have sold %s %s to %s for %s silver."
                        % (
                            buyamount,
                            order.material_name,
                            buyer,
    ...
    def find_brokered_sale_by_id(self, args):
        ...
        try:
            ...
            raise self.BrokerError(
                "Could not find a sale on the broker by the ID %s." % args
            )
