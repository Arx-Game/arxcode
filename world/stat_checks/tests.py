...
from world.stat_checks.models import DifficultyRating, RollResult, NaturalRollType, StatWeight
...
class TestCheckCommands(ArxCommandTest):
    def setUp(self):
        ...
        self.marginal = RollResult.objects.create(name="marginal", value=-20, template=template)
        self.okay = RollResult.objects.create(name="okay", value=45, template=template)
        self.omg = RollResult.objects.create(name="a crit yay!!!!", value=300, template=template)
        self.botch = RollResult.objects.create(name="a botch - boo!!!", value=-100, template=template)
        ...
        NaturalRollType.objects.create(name="botch", value=2, value_type=NaturalRollType.UPPER_BOUND,
                                       result_shift=-2)
    ...
    def test_stat_check_cmd_contest(self, mock_randint):
        ...
        self.call(self.instance, "/contest", "You are not GMing an event in this room.", caller=self.char2)
        self.call_cmd("/contest", "You must specify the names of characters for the contest.")
        self.call_cmd(f"/contest {self.char1},foo=dex + running at normal", "Could not find 'foo'.|Nothing found.")
        mock_randint.return_value = 25
        ...
        valid_cmdstrings = ["/contest/here dex + melee at easy",
