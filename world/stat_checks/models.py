...
class NameIntegerLookupModel(SharedMemoryModel):
    ...
    value = models.SmallIntegerField(verbose_name="minimum value for this difficulty range/rating",
                                     unique=True)
    ...
    def get_all_instances(cls):
        ...
        cls._cache_set = True
        cls._name_to_id_map = {instance.name.lower(): instance.id for instance in values}
        return values
...
class StatWeight(SharedMemoryModel):
    ...
    STAT_CHOICES = ((SKILL, "skill"), (STAT, "stat"), (ABILITY, "ability"), (KNACK, "knack"),
                    (ONLY_STAT, "stat with no skill"), (HEALTH_STA, "health for stamina"),
                    (HEALTH_BOSS, "health for boss rating"))
    stat_type = models.PositiveSmallIntegerField(choices=STAT_CHOICES, default=SKILL)
    level = models.PositiveSmallIntegerField(default=0, verbose_name="minimum level of stat for this weight",
    ...
    weight = models.SmallIntegerField(default=1, verbose_name="weight for this level of the stat",
    ...
    def get_weighted_value_for_type(cls, level: int, stat_type: int) -> int:
        ...
        matches = [ob for ob in weights if ob.stat_type == stat_type and ob.level <= level]
        total = 0
...
class NaturalRollType(NameIntegerLookupModel):
    ...
    value_type = models.PositiveSmallIntegerField("The type of boundary for value", choices=BOUNDARY_CHOICES,
                                                  default=LOWER_BOUND,
    ...
    result_shift = models.SmallIntegerField(default=0,
    ...
    def get_roll_type(cls, roll: int) -> Union[None, "NaturalRollType"]:
        ...
        crits = [ob for ob in instances if ob.value_type == cls.LOWER_BOUND and roll >= ob.value]
        if crits:
            return crits[-1]
        # check if roll was low enough to be a botch. Get lowest botch it passed
        botches = [ob for ob in instances if ob.value_type == cls.UPPER_BOUND and roll <= ob.value]
        if botches:
            return botches[0]
...
class RollResult(NameIntegerLookupModel):
    ...
    template = models.TextField(help_text="A jinja2 template string that will be output with "
                                          "the message for this result. 'character' is the context variable "
                                          "for the roller: eg: '{{character}} fumbles.'")

    @classmethod
    def get_instance_for_roll(cls, roll: int, natural_roll_type: Union["NaturalRollType", None] = None):
        instances = sorted(cls.get_all_instances(), key=lambda x: x.value)
        if not instances:
            raise ValueError("No ResultMessage objects have yet been defined in the database.")
        ...
        closest = max([ob for ob in instances if ob.value <= roll],
...
class DamageRating(NameIntegerLookupModel):
    ...
    armor_cap = models.SmallIntegerField(help_text="Percent of damage armor can prevent. 100 means armor can completely"
                                         " negate the attack.")
