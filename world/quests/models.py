...
class Quest(QuestText):
    ...
    entities = models.ManyToManyField(
        to="dominion.AssetOwner", through="QuestStatus", related_name="quests"
    )
    search_tags = models.ManyToManyField(
        "character.SearchTag", blank=True, db_index=True, related_name="quests"
    )
...
class QuestStep(QuestText):
    ...
    quest = models.ForeignKey(
        to="Quest", related_name="steps", on_delete=models.CASCADE, blank=False
    )
...
class QuestStatus(QuestText):
    ...
    quest = models.ForeignKey(
        to="Quest", related_name="statuses", on_delete=models.CASCADE, blank=False
    )
    ...
    quest_completed = models.DateField(
...
class QuestEffort(SharedMemoryModel):
    ...
    attempt_number = models.PositiveSmallIntegerField(
        verbose_name="Attempt #", blank=True, null=True, help_text="Reorders efforts"
    )
    step_completed = models.BooleanField(
    ...
    def save(self, *args, **kwargs):
        if self.attempt_number is None:
            last_number = (
                self.status.efforts.filter(step=self.step).exclude(id=self.id).count()
            )
            self.attempt_number = last_number + 1
        super().save(*args, **kwargs)
        successful_efforts = QuestEffort.objects.filter(
            status=self.status, step_completed=True
        )
        incomplete_steps = (
            self.step.quest.steps.exclude(efforts__in=successful_efforts)
            .distinct()
            .count()
        )
        if not self.step_completed and self.status.quest_completed and incomplete_steps:
            ...
            self.step_completed
            and not self.status.quest_completed
            and not incomplete_steps
        ):
