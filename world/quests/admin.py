    ...
    def queryset(self, request, queryset):
        if self.value() == "org":
            return queryset.filter(
                entities__organization_owner__isnull=False
            ).distinct()
        if self.value() == "char":
...
class QuestAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        ...
        "=id",
        "search_tags__name",
        "=entities__player__player__username",
...
class QuestEffortInline(admin.StackedInline):
    ...
    fk_name = "status"
    fieldsets = [
        (None, {"fields": [("step", "attempt_number", "step_completed")]}),
        (
            None,
            {
                "fields": [
    ...
    def get_formset(self, request, obj=None, **kwargs):
        ...
        if obj:
            ...
            step_field.queryset = QuestStep.objects.filter(quest=obj.quest_id).order_by(
                "step_number"
            )
        return formset
...
class QuestStatusAdmin(admin.ModelAdmin, StatusDateMixin):
    ...
    search_fields = (
        "=id",
        "quest__name",
        "=entity__player__player__username",
        "=entity__organization_owner__name",
    )
    ...
    fieldsets = [
        (None, {"fields": [("quest", "entity"), ("started", "completed")]}),
        (
            None,
            {
                "fields": ["ic_desc", "gm_note"],
