...
def get_haven_list(request):
    ...
    for haven in Shardhaven.objects.all():
        haven_data = {"id": haven.id, "name": str(haven.name)}
...
def get_obstacle_list(request):
    ...
    if not haven_id:
        return JsonErrorResponse(
            "No haven ID given.", status=404, code=JSON_ERROR_BADPARAM
        )
    ...
    try:
        ...
        return JsonErrorResponse(
            "No such haven found.", status=404, code=JSON_ERROR_BADPARAM
        )

    obstacles = ShardhavenObstacle.objects.filter(
        haven_types__in=[haven.haven_type],
    ...
    for obstacle in obstacles:
        ...
        obstacle_data = {"id": obstacle.id, "name": name}
...
def get_monster_list(request):
    ...
    for monster in monsters:
        monster_data = {"id": monster.id, "name": monster.name}
...
def get_puzzle_list(request):
    ...
    for puzzle in puzzles:
        puzzle_data = {"id": puzzle.id, "name": puzzle.name}
...
def create_room(request):
    ...
    if not layout:
        return JsonErrorResponse(
            "No layout for that haven.", status=404, code=JSON_ERROR_BADPARAM
        )

    if haven.entrance.room is not None:
        return JsonErrorResponse(
            "You cannot add or remove rooms while a haven is instanciated.",
            status=500,
    ...
    if not x_string or not y_string:
        return JsonErrorResponse(
            "Missing a required parameter.", status=500, code=JSON_ERROR_BADPARAM
        )
    ...
    try:
        ...
        return JsonErrorResponse(
            "Invalid parameter.", status=500, code=JSON_ERROR_BADPARAM
        )

    if x == layout.entrance_x and y == layout.entrance_y:
        return JsonErrorResponse(
            "You cannot delete the entrance.", status=500, code=JSON_ERROR_BADPARAM
        )

    if not layout.create_square(x, y):
        return JsonErrorResponse(
            "Unable to delete room that doesn't exit",
            status=500,
            code=JSON_ERROR_BADPARAM,
        )
...
def obstacle_for_id(obstacle_id):
    ...
    try:
        ...
        ShardhavenObstacle.DoesNotExist,
        ShardhavenObstacle.MultipleObjectsReturned,
    ):
        return None
...
def save_room(request):
    ...
    if request.POST.get("obstacle_north"):
        ...
        if not obstacle_north:
            return JsonErrorResponse(
                "Unable to set obstacle", status=500, code=JSON_ERROR_BADPARAM
            )
    ...
    if request.POST.get("monster"):
        ...
        if not monster:
            return JsonErrorResponse(
                "Unable to set monster", status=500, code=JSON_ERROR_BADPARAM
            )
    ...
    if request.POST.get("puzzle"):
        ...
        if not puzzle:
            return JsonErrorResponse(
                "Unable to set puzzle", status=500, code=JSON_ERROR_BADPARAM
            )
