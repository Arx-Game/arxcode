...
class ShardhavenRoom(ArxRoom):
    def extra_status_string(self, looker):
        ...
        if haven_room and haven_room.puzzle and not haven_room.puzzle_solved:
            return (
                "|/|/|yThere is a puzzle to solve here.  Type 'puzzle' for details!|n|/"
            )
    ...
    def shardhaven_square(self):
        try:
            haven_square = ShardhavenLayoutSquare.objects.get(
                pk=self.db.haven_square_id
            )
            return haven_square
        except (
            ShardhavenLayoutSquare.DoesNotExist,
            ShardhavenLayoutSquare.MultipleObjectsReturned,
        ):
            return None
    ...
    def at_object_receive(self, obj, source_location):
        ...
        if (
            ...
            and not haven_square.monster_defeated
            and len(monsters) == 0
        ):
    ...
    def at_object_leave(self, obj, target_location):
        if (
            obj.has_account or (hasattr(obj, "is_character") and obj.is_character)
        ...
        for testobj in self.contents:
            if testobj.has_account or (
                ...
                or testobj.is_typeclass(
                    "world.dominion.dominion_typeclasses.CraftingMaterialObject"
                )
            ):
                testobj.softdelete()
            elif not testobj.is_typeclass(
