...
class WorkingForm(forms.Paxform):
    ...
    name = fields.TextField(
        required=True, max_length=60, full_name="Name", priority=110
    )
    quiet = fields.ChoiceField(
        required=True,
        default=Working.QUIET_NONE,
        choices=Working.QUIET_TYPES,
        priority=105,
    )
    template = fields.BooleanField(
        required=True, default=False, full_name="Use as Template", priority=104
    )
    ...
    weave_alignment = AlignmentField(
        required=False, full_name="Alignment (Weave)", priority=86
    )
    weave_affinity = AffinityField(
        required=False, full_name="Affinity (Weave)", priority=85
    )
    ...
    def validate(self, caller, values):
        if values.get("spell"):
            if (
                values.get("weave_effect")
                or values.get("weave_alignment")
                or values.get("weave_affinity")
            ):
                return "You cannot set both a spell and weave effect!"

        if (
            not values.get("spell")
            and not values.get("weave_effect")
            and not values.get("intent")
        ):
    ...
    def submit(self, caller, values):
        ...
        working = Working.objects.create(
            ...
            template=template,
            intent=intent,
            weave_alignment=alignment,
            weave_affinity=affinity,
            weave_effect=effect,
            spell=spell,
            target_string=target,
        )
        working.add_practitioner(
            practitioner, tool=tool, familiar=familiar, accepted=True
        )
        working.save()

        if template:
            caller.msg(
                "Template named '{}' created; you can now use this with the 'cast' command.".format(
                    name
                )
            )
        else:
            caller.msg(
                "Working created with ID {}.  You can use the 'working' command to invite others.".format(
                    working.id
                )
            )
