...
class TestMagicConditions(ArxTest):
    ...
    def setUp(self):
        ...
        self.weather1 = WeatherType.objects.create(name='MagicTest1', gm_notes='Test weather')
        self.emit1 = WeatherEmit.objects.create(weather=self.weather1,
                                                text='MagicTest1 weather happens.')
        self.weather2 = WeatherType.objects.create(name='MagicTest2', gm_notes='Test weather')
        self.emit2 = WeatherEmit.objects.create(weather=self.weather2,
                                                text='MagicTest2 weather happens.')
        ServerConfig.objects.conf('weather_type_current', value=self.weather1.id)

    def test_weather_condition(self):
        handler = ConditionalHandler("require:weather(MagicTest1);prohibit:weather(MagicTest2)")
        ...
        handler = ConditionalHandler("require:weather(MagicTest2);prohibit:weather(MagicTest1)")
...
class TestMagicSystem(ArxMagicTest):
    ...
    def test_object_mixins(self):
        ...
        self.assertEqual(self.test_object.magic_description, "Once a spectacular glow, now only a spark.")
        self.test_object.db.magic_desc_short = "this object was used for testing"
        self.assertEqual(self.test_object.magic_description, "Once a spectacular glow, now only a spark, this "
                                                             "object was used for testing.")
        self.test_object.db.magic_desc_override = "This is a glowing test object!"
        self.assertEqual(self.test_object.magic_description, "This is a glowing test object!")
    ...
    def test_working(self):
        ...
        self.practitioner.open_node(self.node, SkillNodeResonance.LEARN_FIAT, explanation="Learned by test fiat.")
        ...
        working = Working.objects.create(lead=self.practitioner, spell=self.spell, target_string=self.test_object.name)
        working.add_practitioner(self.practitioner, accepted=True)
        with patch('world.magic.models.Working.successes', new_callable=PropertyMock(return_value=10)) as fake_successes:
            ...
            with patch('typeclasses.scripts.combat.attacks.Attack'):
                ...
                working.finalize()
            self.assertEqual(strip_ansi(working.description_string()),
                             " ID:                 1                                                        \n"
                             " Participants:       Char2                                                    \n"
                             " Lead:               Char2                                                    \n"
                             " Alignment:          Primal                                                   \n"
        ...
        self.assertEqual(pending_magic_text(),
