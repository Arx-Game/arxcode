...
class TestWeeklyEventScript(ArxCommandTest):
    ...
    def add_factors(
        self, owner, vault, lifestyle=1, charm=1, composure=1, command=1, skills=None
    ):
    ...
    def assert_tran_success(self, transaction, sender_vault, receiver_vault=None):
        if receiver_vault:
            self.assertEqual(
                transaction.receiver.vault, receiver_vault + transaction.weekly_amount
            )
        self.assertEqual(
            transaction.sender.vault, sender_vault - transaction.weekly_amount
        )
    ...
    def test_income_basic(self):
        ...
        pl2 = self.add_factors(
            self.assetowner2,
            vault2,
            2,
            charm=5,
            skills={"seduction": 5, "streetwise": 2},
        )
        pl3 = self.add_factors(
            self.assetowner3,
            vault3,
            3,
            charm=6,
            skills={"propaganda": 4, "haggling": 6},
        )
        pl4 = self.add_factors(
            self.assetowner4,
            vault4,
            4,
            command=3,
            skills={"diplomacy": 3, "etiquette": 4},
        )
        pl5 = self.add_factors(
            self.assetowner5,
            vault5,
            composure=4,
            skills={"manipulation": 3, "intimidation": 5},
        )
        ...
        tran1 = new_tran(
            receiver=self.assetowner3,
            sender=self.assetowner2,  # 3 gets paid
            category="Blackmail",
            weekly_amount=10,
        )
        tran2 = new_tran(
            receiver=self.assetowner3,
            sender=self.assetowner5,  # 5 cannot afford
            category="Lost Bet",
            weekly_amount=3333,
        )
        tran3 = new_tran(
            receiver=None, sender=self.assetowner2, category="Debt", weekly_amount=75
        )
        tran4 = new_tran(
            receiver=self.assetowner3,
            sender=self.assetowner4,  # 3 gets paid
            category="Blush",
            weekly_amount=100,
        )
        ...
        self.assert_tran_success(
            tran1, pl2 - tran3.weekly_amount, pl3 + tran4.weekly_amount
        )
        self.assertTrue(
            "Failed payments to you: %s -> %s. Amount: %s"
            % (self.assetowner5, self.assetowner3, tran2.weekly_amount)
