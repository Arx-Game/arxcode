...
class Exit(LockMixins, ObjectMixins, DefaultExit):
    ...
    def can_traverse(self, character):
        if self.destination.check_banned(character):
            ...
            return
        if not self.destination.pets_allowed:
            # will this catch followers? assuming yes, since ban check's here.
            if character.has_animals and not character.player in self.destination.pets_allow_list:
                # TODO: Using player means masked chars may be outed,
                # TODO: unless we just block their pets by default.
                # TODO: Make property to see if character has animals summoned.
                character.msg("Pets are not allowed there.")
                return
