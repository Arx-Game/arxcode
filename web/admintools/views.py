...
def admin_search(request):
    ...
    if not search_term:
        return render(
            request, "admintools/search.html", {"page_title": "Admin Search Tool"}
        )

    rev_qs = Revelation.objects.filter(
        Q(name__icontains=search_term)
        | Q(desc__icontains=search_term)
        | Q(gm_notes__icontains=search_term)
    )

    clue_qs = Clue.objects.filter(
        Q(name__icontains=search_term)
        | Q(desc__icontains=search_term)
        | Q(gm_notes__icontains=search_term)
        | Q(search_tags__name__icontains=search_term)
    )
    clue_qs = clue_qs.distinct()

    crisis_qs = PlotAction.objects.filter(
        Q(actions__icontains=search_term)
