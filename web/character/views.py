...
def sheet(request, object_id):
    ...
    if not show_hidden and (
        hasattr(character, "roster") and character.roster.roster.name == "Unavailable"
    ):
        from django.core.exceptions import PermissionDenied
...
class ChapterListView(ListView):
    ...
    def viewable_crises(self):
        ...
        return Plot.objects.viewable_by_player(self.request.user).filter(
            chapter__in=self.get_queryset()
        )
...
def new_action_view(request, object_id, action_id):
    ...
    if not request.user or not request.user.is_authenticated:
        ...
        or request.user.check_permstring("builders")
        or request.user.char_ob == character()
    ):
...
class ActionForm(AssistForm):
    ...
    def __init__(self, caller, *args, **kwargs):
        ...
        self.fields["category"] = forms.ChoiceField(
            choices=self.CATEGORY_CHOICES, label="What type of action is this?"
        )
...
def edit_action(request, object_id, action_id):
    ...
    initial = {
        "tldr": strip_ansi(real_action.topic),
...
class FlashbackListView(LoginRequiredMixin, CharacterMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not user or not user.is_authenticated:
            raise PermissionDenied
        if user.char_ob != self.character and not (
...
class FlashbackCreateView(LoginRequiredMixin, CharacterMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse(
            "character:list_flashbacks", kwargs={"object_id": self.character.id}
        )
...
class FlashbackAddPostView(LoginRequiredMixin, CharacterMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["allow_add_post"] = bool(
            user_is_staff or flashback.posts_allowed_by(user)
        )
        context["new_post_roll"] = (
            involvement.roll if (context["allow_add_post"] and involvement) else ""
        )
