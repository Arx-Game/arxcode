...
class InvestigationTests(ArxCommandTest):
    def setUp(self):
        ...
        self.clue = Clue.objects.create(
            name="test clue", rating=10, desc="test clue desc"
        )
        self.clue2 = Clue.objects.create(
            name="test clue2", rating=50, desc="test clue2 desc"
        )
        self.revelation = Revelation.objects.create(
            name="test revelation", desc="test rev desc", required_clue_value=60
        )
        self.clue_disco = self.roster_entry.clue_discoveries.create(
            clue=self.clue, message="additional text test"
        )
    ...
    def test_cmd_clues(self, mock_datetime, mock_roster_datetime):
        ...
        inv1 = self.roster_entry2.investigations.create(
            clue_target=self.clue2, active=True
        )
        self.call_cmd("/share 2=Testaccount2", "No clue found by this ID: 2. ")
        self.clue_disco2 = self.roster_entry.clue_discoveries.create(
            clue=self.clue2, message="additional text test2"
        )
        ...
        self.call_cmd(
            template.format("2", "Love Tehom" * 8),
            "You use 101 action points and have 0 remaining this week.|"
            "You have shared the clue(s) 'test clue2' with Char2.\nYour note: {}".format(
                "Love Tehom" * 8
            ),
        )
        self.assertTrue(
            self.account2.informs.filter(
                message="After a recent clue discovery, "
                "%s is no longer active." % inv1
            ).exists()
        )
    ...
    def test_cmd_investigate(self):
        ...
        with patch.object(self.cmd_class, "check_enough_time_left") as fake_method:
            ...
            self.call_cmd(
                "/topic not matching tags",
                "No SearchTag found using 'not matching tags'.",
            )
            self.call_cmd(
                "/topic foo/-bar/zep/-squeeb/merpl",
                "No SearchTag found using 'squeeb'.",
            )
            self.call_cmd(
                "/topic foo/-bar/zep",
                "The tag(s) or clue specified does not match an existing clue, and "
            ...
            self.call_cmd(
                "/story asdf",
                "Creating an investigation: foo; zep; -bar\nasdf\nStat: ??? - Skill: ???",
            )
            self.call_cmd(
                "/finish", "It costs 25 social resources to start a new investigation."
            )
            ...
            self.call_cmd(
                "/finish",
                "You're too busy for such an investigation. (low AP) Try different tags or abort.",
            )
            self.roster_entry.action_points = 300
            self.char1.ndb.confirm_new_clue_write = prompt
            self.call_cmd(
                "/finish",
                "You spend 25 social resources to start a new investigation.|"
            ...
            self.call_cmd(
                "/topic foo/bar/-zep",
                "Creating an investigation: foo; bar; -zep\nstory\n"
                "Stat: ??? - Skill: ???",
            )
            self.call_cmd(
                "/finish",
            ...
            self.call_cmd(
                "/topic clue: another test clue",
                "Creating an investigation: another test clue\nstory\n"
                "Stat: ??? - Skill: ???",
            )
            self.call_cmd(
                "/finish",
            ...
            self.char2.ndb.investigation_form = [
                "clue: 3",
                "story",
                "",
                "",
            ...
            self.call_cmd(
                "/finish",
                "You spend 25 social resources to start a new investigation.|New investigation "
            ...
            self.assertEqual(
                invest3.clue_target.name, "PLACEHOLDER for Investigation #4"
            )
            ...
            self.call_cmd(
                "/requesthelp 4=bob, charlie",
                "You may only invite others to active investigations.",
            )
            self.call_cmd(
                "/active 4", "Char2's investigation on clue: 3 set to active."
            )
            self.assertEqual(self.roster_entry2.action_points, 50)
            self.call_cmd(
                "/requesthelp 4=bob, charlie",
                "No active player found named bob|No active player found named charlie",
            )
...
class SceneCommandTests(ArxCommandTest):
    ...
    def test_cmd_flashback(self, mock_build_msg):
        ...
        self.call_cmd(
            "/create testing", "You have created a new flashback with the ID of #1."
        )
        self.call_cmd(
            "/create testing",
        ...
        self.call_cmd(
            "/post 1=A new testpost", "You have posted to testing: A new testpost"
        )
        self.assertEqual(self.char1.messages.num_flashbacks, 1)
        self.account.inform = Mock()
        self.account2.inform = Mock()
        self.call_cmd(
            "/invite/retro 1=Testaccount2",
            "You have invited Testaccount2 to participate in "
            "flashback #1 with all previous posts visible.",
        )
        self.account2.inform.assert_called_with(
            "You have been invited to participate in flashback #1:" " 'testing'.",
            category="Flashbacks",
        )
        ...
        self.char.msg.assert_called_with(
            "|w[Private Roll]|n %s (Shared with: self-only)"
            % mock_build_msg.return_value,
            options={"roll": True},
        )
        self.call_cmd(
            "/check 1=pleading",
            "Your next post in flashback #1 will use this roll: "
            "%s" % mock_build_msg.return_value,
        )
        self.call_cmd(
            "/post 1=boop.",
            "This roll will accompany the new post: %s\nPlease repeat command to "
            "confirm and continue." % mock_build_msg.return_value,
        )
        self.call_cmd("/post 1=boop.", "You have posted to testing: boop.")
        self.account2.inform.assert_called_with(
            "New post by Char on 'testing' (flashback #1)!", category="Flashbacks"
        )
        self.caller = self.account2
        div = "\n"
        self.call_cmd(
        ...
        self.call_cmd(
            "/summary 1=test", "Only the flashback's owner may use that switch."
        )
        self.call_cmd(
            "/invite 1=Testaccount", "Only the flashback's owner may use that switch."
        )
        self.call_cmd("/conclude 1", "Only the flashback's owner may use that switch.")
        self.call_cmd(
            "/uninvite 1", "You have uninvited Testaccount2 from flashback #1."
        )
        ...
        self.call_cmd(
            ...
            "You have invited Testaccount2 to participate in flashback #1.",
        )
        self.call_cmd(
            "/invite 1",
        ...
        self.call_cmd(
            "/allow 1=Testaccount2,1",
            "Testaccount2 can see 1 previous post(s) in flashback #1.",
        )
        ...
        self.caller = self.account
        self.call_cmd(
            "/allow 1=Testaccount2",
            "Testaccount2 can see all previous post(s) in flashback #1.",
        )
        self.caller = self.account2
        self.call_cmd(
            "1=foo",
        ...
        self.call_cmd(
            "/uninvite 1=Testaccount2",
            "You have uninvited Testaccount2 from flashback #1.",
        )
        self.account2.inform.assert_called_with(
            "You have been retired from flashback #1.", category="Flashbacks"
        )
        ...
        self.account.inform.assert_called_with(
            "Flashback #1 'testing' has reached its conclusion.", category="Flashbacks"
        )
        self.call_cmd(
            "/conclude 1",
            "No ongoing flashback by that ID number.|\n"
            "| ID              | Flashback         | Owner            | New Posts        "
            " ~~~~~~~~~~~~~~~~~+~~~~~~~~~~~~~~~~~~~+~~~~~~~~~~~~~~~~~~+~~~~~~~~~~~~~~~~~~~+\n"
            "| 1               | testing           | Char             | 0",
        )
        self.call_cmd(
            "/create test2", "You have created a new flashback with the ID of #2."
        )
        self.call_cmd("/conclude 2", "Flashback #2 is concluding.")
        self.account.inform.assert_called_with(
            "With no posts, 'test2' (flashback #2) was deleted.", category="Flashbacks"
        )
...
class ViewTests(ArxTest):
    ...
    def test_sheet(self):
        ...
        self.assertEqual(
            self.client.login(username="TestAccount2", password="testpassword"), True
        )
        response = self.client.get(self.char2.get_absolute_url())
        self.assertEqual(response.status_code, 200)
    ...
    def test_view_flashbacks(self):
        response = self.client.get(
            reverse("character:list_flashbacks", kwargs={"object_id": self.char2.id})
        )
    ...
    def test_view_tagged_clues(self):
        ...
        self.roster_entry2.clue_discoveries.create(
            clue=clue, message="additional text test"
        )
        self.assertEqual(
            self.client.login(username="TestAccount2", password="testpassword"), True
        )
        response = self.client.get(
            reverse("character:list_clues", kwargs={"object_id": self.char2.id})
        )
    ...
    def test_view_totals_in_actions(self):
        ...
        action_assistant = PlotActionAssistant.objects.create(
            dompc=self.dompc2, plot_action=action, social=100
        )
        action.save()
        self.assertEqual(
            self.client.login(username="TestAccount2", password="testpassword"), True
        ...
        action_url = reverse(
            "character:view_action",
            kwargs={"object_id": self.char2.id, "action_id": action.id},
        )
...
class PRPClueTests(ArxCommandTest):
    def setUp(self):
        ...
        self.plot.dompc_involvement.create(
            dompc=self.dompc, admin_status=PCPlotInvolvement.GM
        )
    ...
    def test_cmd_prprevelation(self):
        ...
        self.call_cmd(
            "/plot testplot=some notes on stuff",
            "Name: testrev\nDesc: testdesc\nPlot: TestPlot\n"
            "Required Clue Value: 25\nTags: None\nReal: True",
        )
...
class GoalTests(ArxCommandTest):
    ...
    def test_cmd_goals(self):
        ...
        self.call_cmd(
            "/create asdf", "You must provide a summary and a description of your goal."
        )
        self.call_cmd(
            "/create testing/this is a /desc/.", "You have created a new goal: ID #1."
        )
        self.call_cmd(
            "1",
        ...
        self.call_cmd(
            "/status 1=asdf",
            "Invalid Choice. Try one of the following: Succeeded, Failed, Abandoned, "
            "Dormant, Active",
        )
        self.call_cmd(
            "/status 1=dormant", "Old value was: Active\nStatus set to: dormant"
        )
        self.call_cmd(
            "/scope 1=asdf",
            "Invalid Choice. Try one of the following: Heartbreakingly Modest, Modest, "
            "Reasonable, Ambitious, Venomously Ambitious, Megalomanic",
        )
        self.call_cmd(
            "/scope 1=venomously Ambitious",
            "Old value was: Reasonable\nScope set to: venomously Ambitious",
        )
        ...
        self.call_cmd(
            "/rfr 1,1=stuff/things",
            "You have sent in a request for review for goal 1. Ticket ID is 1.",
        )
        self.call_cmd(
            "/rfr 1,1=more stuff/things",
            "You submitted a request for review for goal 1 too recently.",
        )
