...
class TestLoreCommands(ArxCommandTest):
    def setUp(self):
        ...
        self.cat1 = KBCategory.objects.create(title="religion stuff", slug="religion", description="religion desc")
        self.cat2 = KBCategory.objects.create(title="culture stuff", slug="culture", description="culture desc")
        self.subcat1 = KBCategory.objects.create(title="faith stuff", slug="faith", description="faith desc",
                                                 parent=self.cat1)
        self.subcat2 = KBCategory.objects.create(title="god stuff", slug="gods", description="gods desc",
                                                 parent=self.cat1)
        self.item1 = KBItem.objects.create(title="priests", question="can priests marry", answer="lol no",
                                           category=self.cat1)
        self.item2 = KBItem.objects.create(title="archfiends", question="wat dis", answer="bad", category=self.subcat2)
        self.item3 = KBItem.objects.create(title="skald", question="who dis", answer="newgod", category=self.subcat2)
