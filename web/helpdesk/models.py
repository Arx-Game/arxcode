...
class Queue(models.Model):
    ...
    locale = models.CharField(
        ...
        null=True,
        help_text=_(
            "Locale of this queue. All correspondence in this queue will be in this language."
        ),
    )
...
class Ticket(SharedMemoryModel):
    ...
    goal_update = models.ForeignKey(
        "character.GoalUpdate",
        blank=True,
        null=True,
        related_name="tickets",
        on_delete=models.CASCADE,
    )
    kb_category = models.ForeignKey(
        "KBCategory", blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    def _can_be_resolved(self):
        ...
        return (
            ...
            .filter(depends_on__status__in=OPEN_STATUSES)
            .count()
            == 0
        )
...
class TicketChange(models.Model):
    ...
    followup = models.ForeignKey(
        FollowUp, verbose_name=_("Follow-up"), on_delete=models.CASCADE
    )
...
def attachment_path(instance, filename):
    ...
    os.umask(0)
    path = "helpdesk/attachments/%s/%s" % (
        instance.followup.ticket.ticket_for_url,
        instance.followup.id,
    )
...
class PreSetReply(models.Model):
    ...
    name = models.CharField(
        ...
        max_length=100,
        help_text=_(
            "Only used to assist users with selecting a reply - not "
            "shown to the user."
        ),
    )
...
class EscalationExclusion(models.Model):
    ...
    queues = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            "Leave blank for this exclusion to be applied to all "
...
class KBCategory(models.Model):
    ...
    search_tags = models.ManyToManyField(
        "character.SearchTag", blank=True, related_name="kb_categories"
    )
...
class KBItem(models.Model):
    ...
    search_tags = models.ManyToManyField(
        "character.SearchTag", blank=True, related_name="kb_items"
    )
...
class UserSettings(models.Model):
    ...
    settings_pickled = models.TextField(
        ...
        help_text=_(
            "This is a base64-encoded representation of a pickled Python dictionary. Do not change this field via the admin."
        ),
        blank=True,
        null=True,
    )
...
class IgnoreEmail(models.Model):
    ...
    queues = models.ManyToManyField(
        ...
        blank=True,
        help_text=_(
            "Leave blank for this e-mail to be ignored on all "
    ...
    keep_in_mailbox = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Do you want to save emails from this address in the "
...
class CustomField(models.Model):
    ...
    name = models.SlugField(
        _("Field Name"),
        help_text=_(
            "As used in the database and behind the scenes. Must be unique and consist of only lowercase letters with no punctuation."
        ),
        unique=True,
    )
...
class TicketCustomFieldValue(models.Model):
    ...
    field = models.ForeignKey(
        CustomField, verbose_name=_("Field"), on_delete=models.CASCADE
    )
