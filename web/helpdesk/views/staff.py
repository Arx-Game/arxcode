...
def update_ticket(request, ticket_id, public=False):
    ...
    if public and (
        f.comment or (f.new_status in (Ticket.RESOLVED_STATUS, Ticket.CLOSED_STATUS))
    ):
    ...
    return return_to_ticket(
        request.user, helpdesk_settings.HELPDESK_ALLOW_NON_STAFF_TICKET_UPDATE, ticket
    )
...
def ticket_list(request):
    ...
    ticket_paginator = paginator.Paginator(
        ticket_qs, 20  # request.user.usersettings.settings.get('tickets_per_page') or
    )
    try:
    ...
    search_message = ""
    if context.has_key("query") and settings.DATABASES["default"]["ENGINE"].endswith(
        "sqlite"
    ):
        search_message = _(
...
def save_query(request):
    ...
    return HttpResponseRedirect(
        "%s?saved_query=%s" % (reverse("helpdesk_list"), query.id)
    )
...
def ticket_cc_add(request, ticket_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ticketcc.save()
            return HttpResponseRedirect(
                reverse("helpdesk_ticket_cc", kwargs={"ticket_id": ticket.id})
            )
    else:
        form = TicketCCForm()
    return render(
...
def ticket_cc_del(request, ticket_id, cc_id):
    ...
    if request.method == "POST":
        cc.delete()
        return HttpResponseRedirect(
            reverse("helpdesk_ticket_cc", kwargs={"ticket_id": cc.ticket.id})
        )
    return render(
        request,
        "helpdesk/ticket_cc_del.html",
        {
            "cc": cc,
        },
    )
...
def ticket_dependency_del(request, ticket_id, dependency_id):
    ...
    return render(
        request,
        "helpdesk/ticket_dependency_del.html",
        {
...
def calc_basic_ticket_stats(Ticket):
    ...
    ota_le_60_ge_30 = all_open_tickets.filter(
        db_date_created__gte=date_60_str, db_date_created__lte=date_30_str
    )
    ...
    ots.append(
        [
            "< 30 days",
            N_ota_le_30,
            get_color_for_nbr_days(N_ota_le_30),
    ...
    ots.append(
        [
            "30 - 60 days",
            N_ota_le_60_ge_30,
            get_color_for_nbr_days(N_ota_le_60_ge_30),
    ...
    ots.append(
        [
            "> 60 days",
            N_ota_ge_60,
            get_color_for_nbr_days(N_ota_ge_60),
    ...
    all_closed_last_60_days = all_closed_tickets.filter(
        db_date_created__gte=date_60_str
    )
...
def sort_string(begin, end):
    return (
        "sort=db_date_created&date_from=%s&date_to=%s&status=%s&status=%s&status=%s"
        % (
            begin,
            end,
            Ticket.OPEN_STATUS,
