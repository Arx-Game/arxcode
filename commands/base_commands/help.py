...
def format_help_entry(
    title, help_text, aliases=None, suggested=None, unavailable=False, related_tags=None
):
    """
    ...
    if related_tags:
        entries = (
            HelpEntry.objects.exclude(db_key=title)
...
def format_help_list(hdict_cmds, hdict_db, brief=False):
    ...
    if hdict_db and any(hdict_db.values()):
        ...
        for category in sorted(hdict_db.keys()):
            string += "\n\r  {w%s{n:\n" % (str(category).title())
            string += "{G" + ", ".join(
                sorted([str(topic) for topic in hdict_db[category]])
            )
    return string
    ...
    def func(self):
        ...
        if not player.character:
            try:
                char_cmds = [
                    cmd
                    for cmd in player.char_ob.cmdset.all()[0]
        ...
        if len(match) == 1:
            self.msg(
                format_help_entry(
                    match[0].key, match[0].entrytext, suggested=suggestions
                )
            )
            found_match = True
